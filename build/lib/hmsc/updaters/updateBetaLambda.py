...
def updateBetaLambda(params, data, priorHyperparams, dtype=np.float64):
    ...
    if C is None:
      ...
      if len(XE.shape.as_list()) == 2:
        ...
        # A = tf.matmul(iK, tfla.matrix_transpose(Mu)[:,:,None], name="A.1") + tf.einsum("ik,ij->jk", XE, iD*S, name="A.2")[:,:,None]
        A2 = tf.einsum("ik,ij->jk", XE, iD*S, name="A.2")[:,:,None]
      else:
        # iU = iK + tf.einsum("jic,ij,jik->jck", XE, iD, XE, name="iU.2")
        # A = tf.matmul(iK, tfla.matrix_transpose(Mu)[:,:,None], name="A.1") + tf.einsum("jik,ij->jk", XE, iD*S, name="A.2")[:,:,None]
        A2 = tf.einsum("jik,ij->jk", XE, iD*S, name="A.2")[:,:,None]
      A = A1 + A2
      ...
      if len(XE.shape.as_list()) == 2:
        ...
        m0 = tf.matmul(iK, tf.reshape(Mu, [na*ns,1])) + tf.reshape(tf.matmul(XE, iD*S, transpose_a=True), [na*ns,1])
      else:
        XE_iD_XET = tf.einsum("jic,ij,jik->ckj", XE, iD, XE, name="XE_iD_XET")
        m0 = tf.matmul(iK, tf.reshape(Mu, [na*ns,1])) + tf.reshape(tf.einsum("jik,ij->kj", XE, iD*S, name="m0.2"), [na*ns,1])
