...
def logProb(Beta, Gamma, LiV, sigma, EtaList, LambdaList, DeltaList, Y, Loff, X, Tr, Pi, priorHyperparams, rLHyperparams, dtype=np.float64):
  ...
  LoffList = [] if Loff is None else [Loff]
  ...
  L = tf.add_n(LoffList + [LFix] + LRanList)
...
def updateHMC(params, data, priorHyperparams, rLHyperparams, num_leapfrog_steps=10, sample_burnin=0,
    ...
    # log_prob = logProb(Beta, Gamma, LiV, sigma, EtaList, LambdaList, DeltaList, Y, Loff, X, Tr, Pi, priorHyperparams, rLHyperparams, dtype=dtype)
    # tf.print(log_prob)
    # aaa
    ...
    def log_prob_flat(*argv):
      ...
      if updateDelta:
        ...
        offset += nr
      return logProb(Beta, Gamma, LiV, sigma, EtaList, LambdaList, DeltaList, Y, Loff, X, Tr, Pi, priorHyperparams, rLHyperparams, dtype)
