...
```{r set_python_path}
python = file.path(localpath, "hmsc-venv", "bin", "python")  # for Linux and macOS
# python = file.path(localpath, "hmsc-venv", "Scripts", "python")  # for Windows
...
system2(python, "-m pip install git+https://github.com/hmsc-r/hmsc-hpc@simplify-io")
system2(python, "-m pip install --upgrade --no-deps --force-reinstall git+https://github.com/hmsc-r/hmsc-hpc@simplify-io")
```
...
python = file.path(localpath, "hmsc-venv", "bin", "python")  # hmsc-venv for Linux and macOS
# python = file.path(localpath, "hmsc-venv", "Scripts", "python")  # hmsc-venv for Windows
...
First, we shall acquire a sufficiently recent `Hmsc` R package. Most likely, the actual distribution on CRAN is already suitable, but most certainly it can be done from the master branch of `Hmsc` repo on GitHub. `remotes` is a lightweight package that enables it.

```{r install_hmsc_r,  message=FALSE, results='hide'}
install.packages("remotes")
remotes::install_github("hmsc-r/HMSC", force=TRUE)
```
...
```{r fit_model_r, include=TRUE, results='hide'}
...
init_file_path = file.path(localpath, "init_file.rds")
...
```{r create_parallel_python_cmds}
chain_cmd_args_list = vector("list", nChains)
for(cInd in 1:nChains){
	chain_file_path = file.path(localpath, sprintf("post_chain%.2d_file.rds", cInd-1))
