...
def test_updatewNf():
    ...
    for r in range(modelDims["nr"]):
        ...
        assert_allclose(tf.reduce_mean(PsiList[r]), tf.reduce_mean(PsiListTrue[r]), atol=0.005)

        assert_allclose(tf.reduce_mean(PsiList[r]), tf.reduce_mean(PsiListTrue[r]), atol=0.005)
        assert_allclose(tf.reduce_mean(DeltaList[r]), tf.reduce_mean(DeltaListTrue[r]), atol=0.005)
